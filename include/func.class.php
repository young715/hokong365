...
function GetPosStr($cid=0,$id=0,$sign='&nbsp;&gt;&nbsp;')
{
	...
	//设置首页链接
	$pos_str = '<div class="title_zhishi"><a href="'.$cfg_webpath.'">和港健康</a>';
	...
	if(!empty($cid))
	{
		...
		else
		{
			...
			if(isset($r) && is_array($r))
			{
				...
				{
					if(!empty($r['linkurl']))
						return $pos_str.$sign.'<a href="'.$r['linkurl'].'">'.$r['classname'].'</a></div>';
					else
						return $pos_str.$sign.$r['classname'].'</div>';
				}
			}
			else
			{
...
function GetNav($pid=0)
{
	...
	while($row = $dosql->GetArray())
	{
		...
		if($num_nav==6){
			$str .= '<li class="no_border ';
		}else{ 
			$str .= '<li class=" ';
		}
		if($gourl==$self){
			$str .= ' active ';
		}else if($gourl=='knowledge.php' and $self=='article_zs.php'){ 
			$str .= ' active ';
		}else if($gourl=='video.php' and $self=='article_video.php'){ 
			$str .= ' active ';
		}else if($gourl=='share.php' and $self=='article_share.php'){ 
			$str .= ' active ';
		}
...
function GetSubNav($id)
{
	...
	while($row = $dosql->GetArray($id))
	{
		...
		if($row['picurl'] != '')
			...
			$classname = $row['classname'];
		if(!empty($_SESSION['page'])){ $page = $_SESSION['page'];}else{ $page = '';}
		$str .= '<li';
		if($gourl==$self || $gourl == $page)
			$str .= ' class="'.'active'.'"';
